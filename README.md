...
- The icon-led navigation drawer remains scrollable on compact phones, and the Material theme follows the system light/dark setting automatically.
...
- LiteRT is bundled for verified `.tflite` text embeddings plus explicit bounded image/audio tensor tasks (image classification, detection, segmentation, and audio classification), LiteRT-LM is bundled for text generation, the pinned llama.cpp b10079 arm64 CPU pack is bundled for `.gguf` text generation, ONNX Runtime Mobile provides the same bounded tensor workload set for verified `.onnx`/`.ort` files, and ExecuTorch provides it for `.pte` files. These supported engines are already installed with the signed app; users install models directly from Hugging Face, and AndroML selects a compatible engine. The Playground preprocesses images as 224×224 RGB NHWC float32 and PCM16 WAV audio as mono 16 kHz float32; API callers can supply their own validated tensor shape/type. MLC, image generation, and speech-to-text remain unavailable until model-specific signed runtime packs and decoders are shipped; the app never downloads executable engine code outside the signed app-update path or substitutes a fake model result in production.
...
- Local agents can request an allowlisted tool with the bounded `{"tool_call":{"id":"...","arguments":{}}}` envelope; tool approvals are resumable and the API screen exposes a durable hash-only audit history without retaining prompts or results.
